...
impl<F: FillModel> BacktestEngine<F> {
    pub fn new(initial_cash: f64, fill_model: F) -> Self {
        Self { portfolio: Portfolio::new(initial_cash), fill_model, pending_orders: VecDeque::new() }
    }
    pub fn submit_order(&mut self, order: Order) { self.pending_orders.push_back(order); }
    pub fn step(&mut self, event: &MarketEvent) -> Vec<Fill> {
        use crate::market::MarketEventKind::Bar;
        let market_price = match &event.kind { Bar(b) => b.close };
